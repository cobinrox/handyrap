...
# added capability to check for sigint on GPIO 23 and, if detected,
...
class VideoLooper(object):
    ...
    def _display_skipping(self, info):
        """Print  the message Skipping to next movie...
        """
        message = 'Skipping to next movie...'
        self._print(message)
        # Do nothing else if the OSD is turned off.
        if not self._osd:
            return
        ...
        # Clear screen and draw text all
        # centered horizontally and vertically.
        self._screen.fill(self._bgcolor)
        self._screen.blit(label1, ((sw/2-l1w/2),(sw/2-l1w/2)))
        pygame.display.update()
    ...
    def run(self):
        ...
        while self._running:
            #print('g_next_was_clicked: ' + str(g_next_was_clicked))
            # Load and play a new movie if nothing is playing.
            # this is executed every millisecond
            if not self._player.is_playing():
                print("skipping to next movie")
                movie = playlist.get_next()
                #if g_next_was_clicked:
                #    print('Skipping to next movie')
                #    movie = playlist.get_next()
                #    g_next_was_clicked = False
                if movie is not None:
                    # Start playing the first available movie.
                    self._print('Starting up movie on new thread: {0}'.format(movie))
                    self._player.play(movie, loop=playlist.length() == 1, vol = self._sound_vol)
                    self._print('Done starting up movie: {0}'.format(movie))
                    g_next_was_clicked = False
            ...
            if g_next_was_clicked:
                ...
                print('Next was clicked, so stopping player')
                print('************************************')
                print('************************************')
                print('************************************')
                print('************************************')
                self._player.stop(3)
                #g_next_was_clicked = False
                self._display_skipping('skipping')
    ...
    def handle_NEXT_button(self, pin):
        global g_time_stamp 
        global g_nxtCount
        global g_next_was_clicked
        time_now = time.time();
        #if (time_now - g_time_stamp ) > G_DEBOUNCE_TIME:
            # if flag is still true, means that the click wasn't
            # processed/cleared by consumer yet
        if not g_next_was_clicked:
            g_next_was_clicked = True
            g_nxtCount = g_nxtCount + 1
            self._print( "pin [" +  str(pin) + "] B U T T O N    H I T [" + str(g_nxtCount) + "] TIME(S)!!!!")
        else:
            self._print("button click?")
            
        g_time_stamp = time_now
...
#
#
# NEXT button GPIO pin 23
#
G_NEXT_PIN = 23

#
# global time stamp variable used to help debounce buttons
#
g_time_stamp = time.time()
...
#
# debug, num times NEXT button clicked
#
g_nxtCount = 0

#
# NEXT button toggle flag
#
g_next_was_clicked = False
...
if __name__ == '__main__':
    ...
    # Set up interrupt on pin G_NEXT_PIN to be NEXT button
    # (http://raspi.tv/2013/how-to-use-interrupts-with-python-on-the-raspberry-pi-and-rpi-gpio)
    GPIO.setmode(GPIO.BCM)
    # GPIO G_NEXT_PIN as input, pull UP (high) to stop false signals
    GPIO.setup(G_NEXT_PIN, GPIO.IN, pull_up_down=GPIO.PUD_UP)
    GPIO.add_event_detect(G_NEXT_PIN, GPIO.RISING, videolooper.handle_NEXT_button)
